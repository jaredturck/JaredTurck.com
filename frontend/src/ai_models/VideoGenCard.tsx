...
type VideoGenCardProps = {
  ...
  hf_url: string
  desc: string
  videos: { src: string, prompt: string }[]
  params_text?: string
}

export default function VideoGenCard({ title, model_id, hf_url, desc, videos, params_text }: VideoGenCardProps) {
  const { active_index, set_active_index } = use_image_carousel()
  const safe_videos = videos && videos.length ? videos : [{ src: 'https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4', prompt: 'PLACEHOLDER' }]
  const active_prompt = safe_videos[active_index].prompt
  const active_src = safe_videos[active_index].src
  ...
  return (
    <div className='rounded-ui border border-line/40 bg-gradient-to-b from-bg/50 via-surface/16 to-bg/55 backdrop-blur-xl shadow-xl overflow-hidden'>
      <div aria-hidden='true' className='h-1 w-full bg-gradient-to-r from-anyhjs/80 via-accent/35 to-accent-2/55' />
        ...
        <div className='grid gap-5 lg:grid-cols-2 lg:items-start'>
          ...
          <div className='rounded-ui border border-line/30 bg-bg/30 overflow-hidden'>
            <div className='flex items-center justify-between border-b border-line/30 bg-surface/14 px-4 py-3'>
              ...
              <div className='flex items-center gap-2'>
                <button type='button' onClick={() => set_active_index(active_index === 0 ? safe_videos.length - 1 : active_index - 1)} className='h-8 w-10 rounded-ui border border-line/30 bg-surface/18 hover:bg-surface/28 text-sm font-extrabold text-text/90 transition-colors' disabled={safe_videos.length <= 1}>
                  ‹
                </button>
                <button type='button' onClick={() => set_active_index(active_index === safe_videos.length - 1 ? 0 : active_index + 1)} className='h-8 w-10 rounded-ui border border-line/30 bg-surface/18 hover:bg-surface/28 text-sm font-extrabold text-text/90 transition-colors' disabled={safe_videos.length <= 1}>
                  ›
                </button>
              </div>
            </div>
            ...
            <div className='relative'>
              <video key={active_src} src={active_src} controls playsInline muted loop preload='metadata' className='h-80 w-full object-cover object-center' />
            ...
            <div className='flex flex-wrap items-center gap-2 px-4 py-4'>
              ...
              <div className='ml-auto text-xs font-semibold tracking-wide text-muted'>{active_index + 1} / {safe_videos.length}</div>
            </div>
          </div>
        </div>
      </div>
